...
class _KnnModel:
    ...
    def load_data(self):
        ...
        with open("dataCollected.csv") as csv_file:
            csv_reader = csv.DictReader(csv_file)
            for row in csv_reader:
                evidence_list.append(
                    [float(row["meas1"]),#Make the input two remove disease sections
                     float(row["meas2"])
                     ]
                )
                label_list.append(int(row["harmful"]))
    ...
    def train_model(self,evidence, labels):
        ...
        trainModel = KNeighborsClassifier(n_neighbors=3)
